# MW-Interf
Simulations of a nearfield Talbot matterwave interferometer such as the one described in arXiv:2310.05763. 
...
The results then undergo Bayesian inference to make predictions about the parameters of CSL and quantify the amount of parameter space excluded by a value of Shannon Information.
...
5) Optomise.py - Finds the optimum values for $phi_0$ and $t_2$. Can be used as a function from within other code, or can be used on its own when called with a scenario file to optimise and a new scenario file to save the optimum values to
...
2) Prior.py - Generates the prior from the likelihood (where applicable) and the given prior type, possible prior types include:
   ...
   2) Flat
   3) Experimental
3) Updating.py - performs the Bayesian anaylsis from a prior and set of measured data

## Checks Folder
Contains a number of files that we used to check our simulations, included for completeness
1) Bateman_Fig3.py - Reporduces Fig. 3 from arXiv:2310.05763
2) Various Belenchia_Fig files - Reproduce the respective figures from 10.1103/PhysRevA.100.033813
...
6) MieScatter.py - Mie scattering code from the <code>PyMieScatt</code> library with some minor corrections
