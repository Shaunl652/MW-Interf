...
if no_file:
    ...
    for idx,m in enumerate(Ms):
        print(f'\r{idx+1:03}/{len(Ms)} : {m:.2e} AMU',end='')
        expt.mass = m*AMU
    ...
    for idx,m in enumerate(Ms):
        ...
        vals.append(np.exp(sum([lnRs[jdx][1] for jdx,s in enumerate(sources)])))
    np.save(f'{NPYPath}/{sys.argv[2]}.npy',np.array(vals))
    with open(f'NPYs/Decoherence/{sys.argv[1]}.json','w') as f:
